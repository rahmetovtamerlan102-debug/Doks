...
def force_cleanup():
    try:
        ...
        resp_del = requests.post(url_del, json={"drop_pending_updates": True}, timeout=5)
        logger.info(f"deleteWebhook: {resp_del.json()}")
        
        url_upd = f"https://api.telegram.org/bot{BOT_TOKEN}/getUpdates"
        resp_upd = requests.get(url_upd, params={"offset": -1, "timeout": 1}, timeout=5)
        logger.info(f"getUpdates cleanup: {resp_upd.json()}")
...
# ==================== ЗАПУСК С ПРИНУДИТЕЛЬНОЙ ОЧИСТКОЙ ====================
def main():
    force_cleanup()
