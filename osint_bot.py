...
# ==================== API-КЛЮЧИ (ОСТАВЛЕНЫ В КОДЕ, КАК В ФАЙЛЕ) ====================
...
async def vk_get_user(user_id):
    params = {
        ...
        "v": VK_API_VERSION,
        "user_ids": user_id,
        "fields": "first_name,last_name,status,sex,country,city,bdate,photo_max_orig,occupation"
    }
...
async def vk_lookup_220vk(user_id):
    url = f"https://v1.220vk.ru/{user_id}"
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(url, timeout=10) as resp:
                if resp.status == 200:
...
# ----- 5. OFDATA -----
async def ofdata_search(query):
    url = "https://api.ofdata.ru/search"
    headers = {"Authorization": f"Bearer {OFDATA_API_KEY}"}
    params = {"q": query}
    async with aiohttp.ClientSession() as session:
        return await make_request(session, url, params=params, headers=headers)
...
async def sherlock_tg_lookup(username):
    url = f"https://sherlock.tg/api/v2/lookup?username={username}"
    async with aiohttp.ClientSession() as session:
        return await make_request(session, url)

async def discord_intel_resolver(user_id):
    url = f"https://discord-intel.io/resolver/{user_id}"
    async with aiohttp.ClientSession() as session:
        return await make_request(session, url)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "Команды:\n"
        "/vk <id|username> – VK данные\n"
        "/looka <id> – looka.one\n"
        "/murix <id> – murix.ru\n"
        "/snusbase <email> – утечки\n"
        "/numverify <phone> – проверка номера\n"
        "/smsc <phone> – оператор\n"
        "/veriphone <phone> – верификация\n"
        "/ip <ip> – геолокация (5 сервисов) + HudsonRock\n"
        "/email <email> – psbdmp утечки\n"
        "/domain <domain> – утечки по домену\n"
        "/proxynova <query> – поиск комбинаций\n"
        "/hudsonuser <username> – HudsonRock по юзеру\n"
        "/breach <query> – breachdb\n"
        "/phone <phone> – все телефонные сервисы\n"
        "/all <email> – мультипоиск"
    )
...
async def cmd_ip(update, context):
    ...
    await update.message.reply_text(f"🌍 IP {ip}\n{output[:4000] or 'Нет данных'}", parse_mode="HTML")
