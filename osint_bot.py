...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = "🤖 OSINT Bot\n/vk, /looka, /murix, /snusbase, /numverify, /smsc, /veriphone, /ip, /email, /domain, /proxynova, /hudsonuser, /breach, /phone, /all"
...
async def cmd_ip(update, context):
    ...
    tasks = [ipgeolocation_lookup(ip), ip2location_lookup(ip), ipleak_lookup(ip), sypexgeo_lookup(ip), geoplugin_lookup(ip), hudsonrock_ip(ip)]
...
async def cmd_phone(update, context):
    ...
    nv, sc, vp, pd = await asyncio.gather(numverify_lookup(phone), smsc_lookup(phone), veriphone_check(phone), phonedb_lookup(phone))
    out = ""
...
async def cmd_all(update, context):
    ...
    sn, ps, pn, br = await asyncio.gather(snusbase_search(email), psbdmp_email(email), proxynova_search(email), breachdb_search(email))
    res = {}
